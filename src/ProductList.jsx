...
import { addItem } from './CartSlice'; // Assuming the CartSlice is in the same folder
...
function ProductList() {
    ...
    const handleAddToCart = (plant) => {
        dispatch(addItem({ ...plant, quantity: 1 })); // Add quantity property for initial count
    ...
    // Show plants when navigating back from cart
    const handlePlantsClick = (e) => {
        e.preventDefault();
        setShowCart(false); // Hide the cart when showing plants
    };

    // Function to continue shopping
    const handleContinueShopping = (e) => {
        e.preventDefault();
        setShowCart(false); // Close cart and continue shopping
    };
